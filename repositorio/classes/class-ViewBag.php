 ...
 * STRING: 
 * -> Controller: $this->ViewBag->titulo = "Título";
 * -> View: echo $this->ViewBag->titulo;

 * OBJETO:
 * -> Controller:  $this->ViewBag->pessoa = $pessoa;
 * -> View: echo $this->ViewBag->pessoa->nome;

 * LISTA OBJETO:
 * -> Controller: $this->ViewBag->listapessoa = $listapessoa;
 * -> View: 
 *           foreach ($this->ViewBag->listapessoa as $pessoa) {                        
                 echo $pessoa->nome;    
             }
*/
...
class ViewBag implements Iterator {
    ...
    public function __set($name, $value) {
        ...
        if (isset($this->vars[$name]) && in_array(strtolower($name), self::$reserved)) {
